...
import {
  connect,
  disconnect,
  showConnections as statusConnections,
...
import {
  ...
  showRelatedTables,
  showTableSchema,
} from "./commands/tables.js";
import { showTablePath } from "./commands/path.js";
...
program
  ...
  .argument(
    "[db-name]",
    "Connection name to disconnect (defaults to current connection)",
  )
...
program
  ...
  .action(
    async (
      ...
      options: { tables?: boolean; database?: boolean },
    ) => {
...
program
  ...
  .argument(
    "[json]",
    'Optional JSON string containing "database" and/or "tables"',
  )
...
program
  .command("related")
  .description(
    "Show tables related to the specified table and the linking fields",
  )
  .argument(
    "<table>",
    "Table name to inspect relationships for in current connection",
  )
  .action(async (table: string) => {
...
program
  .command("path")
  .description("Find the shortest relationship path between two tables")
  .argument("<source>", "Source table name")
  .argument("<target>", "Target table name")
  .action(async (source: string, target: string) => {
    await showTablePath(undefined, source, target);
  });
...
program
  ...
  .argument(
    "<sql>",
    "SQL query to execute on current connection (use quotes for complex queries)",
  )
